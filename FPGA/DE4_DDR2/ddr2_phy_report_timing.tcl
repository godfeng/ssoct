...
if {!(($::IP(mem_if_memtype) == "DDR2 SDRAM") || ($::IP(mem_if_memtype) == "DDR3 SDRAM"))} {
...
for {set inst_index 0} {$inst_index != [llength $instance_names]} {incr inst_index} {
	...
	if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache($corename-$::instance_name)] } {
		...
		if {$::IP(mem_if_memtype) == "DDR2 SDRAM"} {
			set assumptions_valid [verify_high_performance_timing_assumptions_ddr2 $::instname pins $::IP(write_deskew_t10) $use_flexible_timing $::family]
...
if {[llength $::dqs_clocks] > 0} {
	set_active_clocks [remove_from_collection [all_clocks] [get_clocks $::dqs_clocks]]
}
if {$batch_mode_en == 1} {
	catch {delete_timing_netlist}
}
