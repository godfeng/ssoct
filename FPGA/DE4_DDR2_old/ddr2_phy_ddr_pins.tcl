...
proc verify_high_performance_timing_assumptions_ddr2 {instname pin_array_name write_deskew_t10 flexible_timing family} {
	upvar 1 $pin_array_name pins
	set num_errors 0
	load_package verify_ddr
	set ck_pins [lsort $pins(ck_p)]
		...
		if {$num_errors == 0} {
			...
			if {$flexible_timing} {
				if {$has_dqsn > 0} {
					...
					set verify_assumptions_exception [catch {verify_assumptions -memory_type ddr2 -read_pins_list $read_pins_list -write_pins_list $write_pins_list -ck_ckn_pairs $ck_ckn_pairs -clk_to_write_d $clk_to_write_d -clk_to_write_clock $clk_to_write_clock -clk_to_ck_ckn $clk_to_ck_ckn -mimic_pin [lindex $ck_pins 0] -dll $dll_name -flexible} verify_assumptions_result]
				}
			} else {
				if {$has_dqsn > 0} {
					set verify_assumptions_exception [catch {verify_assumptions -memory_type ddr2 -read_pins_list $read_pins_list -write_pins_list $write_pins_list -ck_ckn_pairs $ck_ckn_pairs -clk_to_write_d $clk_to_write_d -clk_to_write_clock $clk_to_write_clock -clk_to_ck_ckn $clk_to_ck_ckn -mimic_pin [lindex $ck_pins 0] -dll $dll_name -read_clock_pairs $read_clock_pairs -write_clock_pairs $write_clock_pairs} verify_assumptions_result]
				} else {
					set verify_assumptions_exception [catch {verify_assumptions -memory_type ddr2 -read_pins_list $read_pins_list -write_pins_list $write_pins_list -ck_ckn_pairs $ck_ckn_pairs -clk_to_write_d $clk_to_write_d -clk_to_write_clock $clk_to_write_clock -clk_to_ck_ckn $clk_to_ck_ckn -mimic_pin [lindex $ck_pins 0] -dll $dll_name} verify_assumptions_result]
				}
			}
			if {$verify_assumptions_exception == 0} {
				incr num_errors [lindex $verify_assumptions_result 0]
...
# ----------------------------------------------------------------
	...
	set patterns [list]
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_*inst|*adc|*addr\[*\].addr_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_*inst|*adc|*ba\[*\].ba_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_*inst|*adc|*cas_n_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_*inst|*adc|*ras_n_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_*inst|*adc|*we_n_struct|*addr_pin|dataout
