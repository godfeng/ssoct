...
%    function [s,c] = sin400k_model(phi_inc_i,phase_mod_i,freq_mod_i)
...
if(nargin==0)
 fprintf('Error using sin400k_model : Not enough input arguments\n');
else
 N=length(phi_inc_i);
end
if(nargin==1)
...
elseif(nargin==2)
 if(length(phase_mod_i)~=N)
   fprintf('Error using sin400k_model : input vector length mismatch\n');
 else
   freq_mod_i=zeros(1,N);
 end
elseif(nargin==3)
 ...
 fprintf('Error using sin400k_model : Incorrect number of input arguments\n');
end
N=length(phi_inc_i);
numch = 1.0;
apr = 32.0;
