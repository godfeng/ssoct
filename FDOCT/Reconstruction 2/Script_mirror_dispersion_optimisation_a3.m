...
target_peak=7;
clear FWHM_matrix a
a(:,2)=[0 20.6624]%1.1794e-5;%[-10:1:0 0.35:0.1:1.17 (1.1785:0.0001:1.18) 1.18:0.1:2 3:1:10]*1e-5;
a(:,3)=[0 -32.0993]*1e-8%[-0.81:0.0001:-0.808]*1e-8;
...
if number_of_combinations>1
    ...
    figure(200);plot(a(:,3),best_resolution_at_depth)
    title('Position of best FWHM depending on a_3 value')
    xlabel('a_3 value')
    ylabel('Depth with best axial resolution (um)')
    
    figure(300);plot(a(:,3),mean_FWHM*P(1))
    title('Mean FWHM of all peaks')
    xlabel('a_3 value')
    ylabel('Mean resolution (um)')
    
    best_a3=a(find(meaningful_mean_FWHM==min(meaningful_mean_FWHM)),3);
    figure(400);plot(a(:,3),meaningful_mean_FWHM*P(1))
    title({['Mean FWHM of the peaks in the '...
        ...
        ['Best a_3 value = ' num2str(best_a3)]})
    xlabel('a_3 value')
    ylabel('Mean resolution (um)')
