...
% using a the signal from a mirror which is deeper than the former version
%% Load and prepare data
clear; close all; clc
% load (fullfile('D:\Edgar\ssoct\Matlab\Misc','coupler_50_50.mat'))
load (fullfile('D:\Edgar\ssoct\Matlab\Misc','phase.mat'))
% clear B ref data.Aline data.refB
[rawBscanOut ref_hi] = browseVolume(100,'D:\Edgar\ssoct\Matlab\Acquisition\DATA\2012_05_25_Mirror_2mm\17_29_55_2mm\17_30_45_0000001.dat');
B_hi = squeeze(rawBscanOut(:,:,100));
...
B_hi_corr = mean(B_hi_corr(:, 102:118),2);
figure(1)
plot(fixedSampling, B_hi_corr, fixedSampling, B_hi(:,102), fixedSampling, ref_hi_filt)
legend({'Reference Corrected' 'Raw' 'Reference'}, 'location', 'SouthEast')

%% Theoretical sinus zero-crossing
clc
A_theo = 3000*cos(2*pi*0.253*fixedSampling - 0.4);
...
% Display results
fprintf('Number of theoretical zero-crossings: %.d \n',numel(ind_theo))
fprintf('Number of experimental zero-crossings: %.d \n',numel(ind_corr))
...
t_corr_res = t_corr_res(extraElements:end-extraElements-1);
% figure(31);
% plot(t_corr_res, 'b-')
...
legend({'\DeltaT Theo.' '\DeltaT Raw' '\DeltaT Exp. Interp.'}, 'location', 'SouthEast')
...
fprintf('Interpolated FWHM: %.1f um\n',FWHM_um)
...
legend({'\phi Interp.' '\phi Raw' '\phi Theo.'},'location','southeast')
tilefigs
