...
% Resampling (Interpolation/Decimation) along columns (A-lines)
Aline_corr = interp1(k0, Aline_corr, fixedSampling, 'linear');
figure(1); plot(Aline_corr,'r-')
...
Afft = abs(fftshift(fft(Aline_corr,NFFT,1),1));
Rfft = abs(fftshift(fft(ref,NFFT,1),1));
%figure(2); plot(Afft,'k-')

% Keep only positive half
Afft_half = Afft(NFFT/2+1:end);
% figure(3); plot(Afft_half,'k-')
...
xlim ([0 500e-6])
legend({'Experimental' 'Exp FWHM' 'Theoretical' 'Theo FWHM'})
