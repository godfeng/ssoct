...
if isbitmap(formats)
    ...
    for a = {'png', 'bmp'}
        if formats.(a{1})
            imwrite(A, [name '.' a{1}], 'XResolution', 1200, 'YResolution', 1200);
        end
    end
    % Save tif file
    if formats.tif
        imwrite(A, [name '.tif'], 'Compression', 'lzw', 'Resolution', [1200 1200]);
    end
...
for a = 1:nargin-1
    ...
    elseif ischar(varargin{a})
        if numel(varargin{a}) == 4 && varargin{a}(1) == '-' && ~isempty(intersect(varargin{a}(2:4), {'pdf', 'eps', 'png', 'tif', 'jpg', 'bmp'}))
            formats.(lower(varargin{a}(2:4))) = true;
        else
            name = varargin{a};
            if numel(name) > 3 && name(end-3) == '.' && ~isempty(intersect(name(end-2:end), {'pdf', 'eps', 'png', 'tif', 'jpg', 'bmp'}))
